    ...
    <Element name="faq" className="relative">
      ...
      <div className= "faq-glow_before relative z-2 border-2 border-s2 bg-s1">
        <div className="container flex gap-10 max-lg:block">
          ...
          <div className="ralative flex-1 pt-24">
            {faq.slice(0, halfLength).map((item, index) => (
              <div  className="faq-item_first relative mb-10 max-lg:mb-8">
                <FaqIteam key={item.id} item={item} index={index}/>
              </div>
            ))}
          </div>

          <div className="ralative flex-1 lg:pt-24">
            {faq.slice(halfLength).map((item, index) => (
              <div  className="faq-item_first relative mb-10 max-lg:mb-8">
                <FaqIteam key={item.id} item={item} index={halfLength + index}/>
              </div>
            ))}
          </div>
